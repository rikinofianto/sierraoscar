...
class medoo{
    ...
    protected $database_name='sierra_oscar';
    ...
    function __construct($options=null){
        try{
            ...
            switch($type){
                ...
                case 'pgsql':
                $dsn=$type.':host='.$this->server.($is_port?';port='.$port:'').';dbname='.$this->database_name;break;case 'sybase':$dsn='dblib:host='.$this->server.($is_port?':'.$port:'').';dbname='.$this->database_name;
                break;

                case 'oracle':
                $dbname=$this->server?'//'.$this->server.($is_port?':'.$port:':1521').'/'.$this->database_name:$this->database_name;$dsn='oci:dbname='.$dbname.($this->charset?';charset='.$this->charset:'');
                break;

                case 'mssql':$dsn=strstr(PHP_OS,'WIN')?'sqlsrv:server='.$this->server.($is_port?','.$port:'').';database='.$this->database_name:'dblib:host='.$this->server.($is_port?':'.$port:'').';dbname='.$this->database_name;$commands[]='SET QUOTED_IDENTIFIER ON';
                break;

                case 'sqlite':
                $dsn=$type.':'.$this->database_file;$this->username=null;$this->password=null;
                break;
            }
    ...
    protected function data_implode($data,$conjunctor,$outer_conjunctor=null){
        ...
        foreach($data as $key=>$value){
            ...
            if(preg_match("/^(AND|OR)(\s+#.*)?$/i",$key,$relation_match)&&$type=='array'){
                ...
                if(isset($match[4])){$operator=$match[4];
                    if($operator=='!'){
                        switch($type){
                            case 'NULL':
                    ...
                    switch($type){
                        ...
                        break;
                        case 'string':
                        $wheres[]=$column.' = '.$this->fn_quote($key,$value);break;
                    }
                }
            }
        }
    ...
    public function replace($table,$columns,$search=null,$replace=null,$where=null){
        if(is_array($columns)){
            ...
            $replace_query=implode(', ',$replace_query);$where=$search;
        } else {
            if(is_array($search)){
                $replace_query=array();
    ...
    public function has($table,$join,$where=null){$column=null;
    ...
    public function min($table,$join,$column=null,$where=null){
        ...
        if($query){$min=$query->fetchColumn();
    ...
    public function sum($table,$join,$column=null,$where=null){$query=$this->query($this->select_context($table,$join,$column,$where,'SUM'));
        return $query?0+$query->fetchColumn():false;
    }

    public function debug(){$this->debug_mode=true;
            return $this;
    }
